...
`20260912-005925-643` 在常驻 Editor PID 15476 的 AssetDatabase.Refresh 后停止响应编排轮询，没有场景审计、Play Mode 或游戏结果；360 秒外部期限到达后按环境失败保留。Editor 未关闭，仅把其超时请求显式禁用，防止未来恢复时误启动旧回合。使用已结束的回归工作区导入缓存准备 `AnomalySearchFinal`，后续复用新会话；此故障不记为 2731 游戏逻辑失败，也不宣称已定位 Editor 原生挂起根因。

新会话 `20260912-010600-466`：2731 的 4× 完整回合 **PASS**，65.05 秒、5955 帧、5954 实际渲染，0 程序错误、0 失败指令、0 停滞。开局群记录 30 个敌人，两个角色自主撤离，9 次背包会话，22 件结算物品逐种类/数量相等；最后可见血量 Agent 1 为 169、Agent 2 为 74。本轮自然出现反击恢复，所有已定义覆盖项为 true。当前修复、定向构造、原失败种子复跑和审查闭环完成，提交为 `b204253`；P5 继续在同一新场景和会话完成其余冻结矩阵。
