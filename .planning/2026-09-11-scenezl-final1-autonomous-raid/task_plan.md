...
日期：2026-09-11。状态：**用户已授权自主实施。层级/RaidFlow 修复、P0 基线、P1 探针、P2 自动背包首轮、P4a–P4c 性能首轮已完成实现和定向测试。P4d 按最新要求将 Cluster 范围刷新限制为 0.05 实际秒，记录见下；完整自主搜打撤、120 FPS 和最终环境验收仍未通过。**
...
5. 120 FPS 是整局目标，不能用只站在开场、空场景、关闭渲染或平均 FPS 代替。先锁定设备、分辨率、画质，再讨论是否达标。
...
7. **逻辑测试允许加速，性能测量使用正常速度**：用户已允许逻辑测试加速，先验证 2 倍速，稳定后可用 4 倍速；关键时间/物理边界保留 1 倍速回归，120 FPS 验收固定 1 倍速。移动/战斗仍走游戏时间，不手动推进 Physics；保留原 fixedDeltaTime 的游戏时间步长，避免靠增大物理步长加速。背包搜索仍走正式 unscaled 时间，进程硬期限用单调墙钟。
...
| PerformanceRaid | 相同规则、原内容、真实渲染，降低诊断成本，逐帧记录时间 | 120 FPS 门禁；不能只挑轻负载片段 |
...
用户已确认以下组合用于检查“超过 120 FPS”。它同时检查平均表现、持续低帧和单帧卡顿上限，不以平均 FPS 单项代替验收。

| 指标 | 建议主门槛 |
| --- | --- |
| 整体吞吐 | 有效游戏区间平均 FPS >120，计算为帧数/真实持续时间，不平均逐帧 FPS |
| 持续低帧 | 每个滑动 1 秒窗口 FPS >120，采样步长 0.25 秒 |
| 尾部帧 | P99 帧间隔 <8.333 ms；1% Low >120 FPS，定义为最慢 1% 帧耗时均值的倒数 |
| 卡顿上限 | 可交互阶段单帧不超过 16.667 ms；所有 >8.333 ms 的帧保留，输出数量、最长连续段、阶段和来源；任何 >33.333 ms 都单列严重卡顿 |
...
“逐帧永远 >120”与“稳定超过 120”是不同门槛。若用户要求字面上的每一帧都超过 120，则把单帧上限收紧到 8.333 ms，而不是降低 P99 标准后声称等价。当前建议采用表中组合，所有未满足项都可见。
...
5. **已确认。** 本机 4K、高画质的测量配置和第 8 节的 120 FPS 门槛；Editor Play Mode 与 Player 各自报告。
