...
日期：2026-09-11，进展更新至 2026-09-12。状态：**按用户最新约定，有限验收矩阵和交付已完成。层级/RaidFlow、P0–P6 及追加的 P3h–P3l、P5c 已完成相应实现和验证。七轮全部通过，5 轮双人撤离，2 轮正常战死结局；正常速度 Editor 平均 95.23 FPS，Player 平均 236.22 FPS，均 >60。小地图优化已取消，战死按正常玩法核对。Editor 保留；历史原生挂起和瞬时 Unreachable 的根因仍有未确认部分，不声称所有历史问题已消失。**

最终结果见 [验收报告](../../outputs/scenezl_final1_validation_report.md)、[机器证据](../../outputs/scenezl_final1_validation.json)和 [P6 交付](p6_delivery.md)。

**最新验收约定：战死是预期玩法结果，之后不因战死进入 bug 修复，不要求每轮全员生还。** 仍检查死亡/撤离终态、存活者结算、程序错误和性能；死亡回合不冒充完整撤离覆盖。自动化分类及请求交接修复见 [P5c](p5c_request_and_outcomes.md)。下文历史规划中“每轮两人撤离”的要求以本约定为准，原始历史报告不回写。
...
5. 用户最新确认性能只要求正常速度完整回合的平均 FPS >60，P99、最低帧、峰值仅诊断，小地图不继续优化。4K 高画质保持，不用开场、空场景或关闭渲染替代真实回合，历史报告保留原口径。
...
用户最新确认只要求平均 FPS 严格大于 60。测量进程仍不限帧，以下尾部数据继续输出但不作为门禁；不再为这些诊断值追加小地图等优化。

| 指标 | 当前验收口径 |
| --- | --- |
| 整体吞吐 | 唯一时间门槛：有效游戏区间平均 FPS >60，计算为帧数/真实持续时间，不平均逐帧 FPS |
| 持续低帧 | 输出滑动 1 秒 FPS，采样步长 0.25 秒，仅诊断 |
| 尾部帧 | 输出 P99、1% Low，仅诊断；1% Low 定义仍为最慢 1% 帧耗时均值的倒数 |
| 卡顿 | 保留全部慢帧和最大帧，16.667/33.333 ms 仅为历史参考，不导致验收失败 |
| 分阶段 | 移动、搜索等待、战斗、撤离和背包暂停按需解释，不设额外阶段 FPS 门槛，不拉长结算画面抬高平均值 |
| 初始化 | 导入/加载单独统计；进入 Play Mode 到可交互的耗时有明确期限，原始开局帧完整保存。首次交战/首次开箱不能临时算进预热而剔除 |

平均值比较保留到小数点后 6 位，避免浮点求和把恰好 60 FPS 误判成大于 60；慢帧原始记录不变。历史组合门槛报告不重写，新口径输出 acceptancePolicy=AverageFpsGreaterThanTarget，原预算字段和尾部值仅用于诊断。
...
以下是早期治理的诊断预算，保留用于理解热点，不再作为独立验收门槛；最终性能只检查完整回合平均 FPS >60。
...
- 三项治理后的必要特效治理已完成；按最新要求停止小地图及仅为尾部值追加的优化。后续仅在平均 FPS 未达标或发现实际行为缺陷时另作具体规划。
- 验收：完整回合平均 FPS >60，玩法、空间、缓存失效和 UI 视觉没有退化；局部预算仅诊断。
...
- **实施结果：最终修复提交 `804e78d` 后重新冻结全部输入，七轮全部通过，5 轮双人撤离，2 轮正常战死结局，五轮 4×、一轮 1× Editor、一轮 1× Player 均无程序错误、失败/拒绝指令或停滞。正常速度 Editor 平均 95.23 FPS，Player 平均 236.22 FPS。每轮初始 30 个敌人均已注册，背包和仓库契约通过。详见 [P5](p5_player_verification.md) 和最终报告；此前失败保留为历史证据。**
...
- **实施结果：验收 Markdown、机器 JSON、README 和最终架构审查均已完成。七轮完整输入指纹一致，原始证据保存 SHA256；用户 FFT 场景差异保留。当前完成仅指已约定的有限配置/种子/路线，不扩张为所有场景或硬件的无缺陷承诺。**
...
| 默认 AI 在原数值下确实会死亡 | 用户明确确认正常战死不进入 bug 修复；核对死亡终态、存活者结算和运行异常，不改生存策略或数值 |
...
## 12. 初始规划交付和 Review 历史

初始提交规划时已完成：读取框架/敌人/渲染文档、旧规划及执行审查，扫描场景、prefab、SO、默认自主链、背包、撤离和工具，结合用户提供的完整 Profiler 数据形成调查和本大规划。当时尚未实现；后续实施状态以上方各阶段结果和最终报告为准。
...
5. **已确认，最新再次调整。** 本机 4K、高画质保持，只要求平均 FPS >60，P99/最低帧/峰值不阻挡验收，小地图不继续优化；Editor Play Mode 与 Player 各自报告。最新口径见 [P4i](p4i_average_fps_scope.md)，历史口径见 [P4e](p4e_60fps_shutdown.md)。
