...
function Test-SceneRaidSettlement {
    param([string]$OutputPath, $Config, [object[]]$Events, $Result, $CarriedEvidence = $null)
    ...
    try {
        ...
        foreach ($id in @('1','2')) {
            ...
            $coverage["ledger:$id"] = $after.Count -gt 0
            $coverage["verifiedTransfer:$id"] = $false
            if ($null -ne $CarriedEvidence -and $extracted -contains $id) {
                if (!$CarriedEvidence.ContainsKey($id) -or !$CarriedEvidence[$id].available) { throw "missing_carried_inventory:$id" }
                $items = Get-RaidItemQuantities $CarriedEvidence[$id].backpack
                $equipment = Get-RaidItemQuantities $CarriedEvidence[$id].equipped
                foreach ($key in $equipment.Keys) { Add-RaidQuantity $items $key $equipment[$key] }
                foreach ($key in $items.Keys) { Add-RaidQuantity $expected $key $items[$key] }
            }
            if ($after.Count -eq 0) {
                if ($completedSessions -gt 0 -or $agentLedgers.Count -gt 0 -or ($null -eq $CarriedEvidence -and $extracted -contains $id)) { $failures.Add("missing_inventory_ledger:$id") }
                continue
            }
            ...
            if ($null -eq $CarriedEvidence -and $extracted -contains $id) {
                foreach ($key in $carried.Keys) { Add-RaidQuantity $expected $key $carried[$key] }
            }
            if ($null -ne $CarriedEvidence) {
                if (!$CarriedEvidence.ContainsKey($id) -or !$CarriedEvidence[$id].available) { throw "missing_carried_inventory:$id" }
                $observed = Get-RaidItemQuantities $CarriedEvidence[$id].backpack
                $equipment = Get-RaidItemQuantities $CarriedEvidence[$id].equipped
                foreach ($key in $equipment.Keys) { Add-RaidQuantity $observed $key $equipment[$key] }
                if (!(Test-RaidQuantitiesEqual $carried $observed)) { throw "carried_ledger_mismatch:$id" }
            }
            ...
            if ($verified -ne $completedSessions -or $verified -ne @($opened | Where-Object agent -eq $id).Count) { throw 'inventory_close_evidence_missing' }
            $coverage["verifiedTransfer:$id"] = $verifiedTransfers -gt 0
        }
...
Export-ModuleMember -Function Test-SceneRaidSettlement,Get-RaidItemQuantities,Test-RaidQuantitiesEqual,Add-RaidQuantity,New-RaidQuantities
