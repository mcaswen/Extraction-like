...
| 自动回归 | 已有构造测试和正式场景运行器 | 已登记 163 个构造用例，按改动选择回归；编码 Agent 自动运行 Play Mode、操作正式背包、核对结算，另有独立 Player 渲染和性能验证 |
...
- 撤离受有效伤害后暂停，反击期间保持当前有效敌人，结束后继续原撤离目标和原指令。连续受击只保留一份恢复记录，新手动命令、取消或死亡清理旧记录。
...
实际物品经过合法放置和堆叠仍无法收纳时，该 Agent 自主转入撤离，箱内剩余物品保留。容量不足按角色记录，另一名有空间的 Agent 仍可继续搜刮。背包会话同时核对角色、资源对象和当前指令，关闭无关界面不会完成该资源。
...
- 一人战死、另一人撤离时会正常结束回合，已撤离者的结算保留；死亡角色不入库。战死属于预期玩法结果，自动验证仍独立检查程序异常和结算一致性。
...
2026-09-11 的 F1–F7 / R1–R5 历史矩阵：**75 例 × 3 轮，225/225 通过**；另有 7 个报告层故障构造通过。顶部指令提示已保存 [成功](outputs/feedback/success.png)及 [失败原因](outputs/feedback/failure.png)的正式 prefab 渲染证据。后续目录扩充到 163 例，按修改范围执行，不能把历史 225/225 当作新增用例或整张场景已通过的证据。
...
│  ├─ Automation/   显式验证构建中的 SceneRaid 观察、背包驱动和采样，普通 Player 不启用
│  ├─ Renderer/     海面、描边与后处理
│  ├─ Audio/        音频播放入口
│  └─ Obsolete/     历史桌游原型
