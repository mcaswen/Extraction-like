...
| Agent 自主行为 | 已接入默认发现与执行链 | 移动、接战、搜索、撤离；风险评分决策模块已实现，但基础预制体默认关闭 |
...
| 画面与反馈 | 已有美术资源及多处玩法接入 | 角色与敌人动画、技能 VFX、HUD、物品图标、音效、Toon 描边和墙体淡出 |
...
- 默认自动发现逻辑会比较可用敌人与资源的距离，并尝试保持有效的资源目标；没有可选敌人／资源时寻找撤离点。
- 玩家手动目标受到保护，自动扫描会暂缓覆盖。受击反击也有独立处理规则；最终是否切换行动由当前指令与状态转移条件共同决定。
- 风险评分模块支持距离、风险、当前目标奖励和低血量撤离倾向，但当前基础 Agent 预制体默认未启用该模块。
...
Agent 接近目标后，优先尝试已就绪技能，再执行普通攻击。流派配置定义攻击与技能，天赋和图腾进一步修正属性。当前正式 Agent 配置集中在冰、土两种流派，已配有四个技能配置：冰系 `FrostAssault`、`Winterfall`，土系 `StoneWall`、`QuakeField`。
...
| `Assets/Prefabs` | 127 个预制体，另有 HUD／结算预制体位于 `Resources` |
...
| `Assets/Scripts` | 364 个 C# 文件，其中 34 个位于顶层 `Editor`、63 个位于 `Obsolete` |
...
历史记录中使用过 Unity 编译、生成项目的 `dotnet build`、静态资源引用检查和人工 Play Mode 验收。当前未发现项目自有的 NUnit／UnityTest 用例；以下可作为内容改动后的人工验收顺序：
